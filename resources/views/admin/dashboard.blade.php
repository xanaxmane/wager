        ...
        <div class="card">
            <div class="card-body pb-0" style="position: relative;">
                ...
                @php
                    ...
                    $fill_data_aggregator = function($days, $aggregator_id, $currency_id) use($fix) {
                        $out = [];
                        for($i = 0; $i < $days; $i++)
                            array_push($out, $fix(\Illuminate\Support\Facades\DB::table('invoices')
                                ...
                                ->where('created_at', '>=', \Carbon\Carbon::today()->subDays($i))
                                ->where('created_at', '<', \Carbon\Carbon::today()->subDays($i - 1))
                                ->where('aggregator', $aggregator_id)->where('status', 3)->sum('sum')));
                        return array_reverse($out);
                    };
                    ...
                    foreach (\App\Payment\Aggregator::list() as $aggregator) {
                        foreach(\App\Currency\Currency::all() as $currency) {
                            array_push($merge, [
                                'name' => "{$aggregator->name()} ({$currency->name()})",
                                'data' => $fill_data_aggregator(7, $aggregator->id(), $currency->id())
                            ]);
                        }
                    }
