                            ...
                            <table class="table table-borderless mb-0 text-muted">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <th>{{ $user->latest_activity->diffForHumans() }}</th>
                                    </tr>
                                    <tr>
            ...
            <div class="card">
                <div class="card-body">
                    <table class="table dt-responsive nowrap">
                        ...
                        <tr>
                            <td>Total</td>
                            <td>{{ \App\Game::where('demo', '!=', true)->where('status', '!=', 'in-progress')->where('status', '!=', 'cancelled')->where('user', $user->_id)->count() }}</td>
                            <td>{{ \App\Game::where('demo', '!=', true)->where('status', 'win')->where('user', $user->_id)->count() }}</td>
                            <td>{{ \App\Game::where('demo', '!=', true)->where('status', 'lose')->where('user', $user->_id)->count() }}</td>
                        </tr>
                        @foreach(\App\Currency\Currency::all() as $currency)
                            <tr>
                                <td>{{ $currency->name() }}</td>
                                <td>{{ \App\Game::where('demo', '!=', true)->where('status', '!=', 'in-progress')->where('status', '!=', 'cancelled')->where('user', $user->_id)->where('currency', $currency->id())->count() }}</td>
                                <td>{{ \App\Game::where('demo', '!=', true)->where('status', 'win')->where('user', $user->_id)->where('currency', $currency->id())->count() }}</td>
                                <td>{{ \App\Game::where('demo', '!=', true)->where('status', 'lose')->where('user', $user->_id)->where('currency', $currency->id())->count() }}</td>
                                <td>{{ number_format(\App\Game::where('demo', '!=', true)->where('status', '!=', 'cancelled')->where('user', $user->_id)->where('currency', $currency->id())->sum('wager'), 8, '.', '') }} {{ $currency->name() }}</td>
                    ...
                    <table id="datatable" class="table dt-responsive nowrap">
                        ...
                        <tbody>
                            @foreach(\App\Game::where('demo', '!=', true)->where('user', $user->_id)->get() as $game)
                                <tr>
                                    <td>{{ $game->game }}</td>
