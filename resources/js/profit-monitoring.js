const stats = {
    ...
    reset: function() {
        ...
        $('[data-profit-monitoring-currency]').attr('class', window.Laravel.currency[$.currency()].icon)
            .css({ color: window.Laravel.currency[$.currency()].style });
    ...
    update: function() {
        ...
        $('#profit').html(this.profit.toFixed(8)).attr('class', `text-${this.profit === 0 ? '' : this.profit > 0 ? 'success' : 'danger'}`);
        $('#wins').html(this.wins);
...
$(document).ready(function() {
    ...
    if(!$.isGuest()) window.Echo.channel('laravel_database_Everyone').listen('LiveFeedGame', (e) => {
        if(e.game.type !== 'multiplayer' || e.game.user !== window.Laravel.userId) return;
        setTimeout(() => $.pushStats(e.game), e.delay);
    });
});

$(document).on('wager:currencyChange', () => $.stats().reset());
