        ...
        $.ajax({
            ...
            error: function(data) {
                if(data.status === 500) {
                    ...
                    console.error(route + arrayToRouteParams(options), data.status, data.responseText);
                    reject(-1);
                } else {
...
$.whisper = function(event, data = {}, timeout = 15000) {
    return new Promise(function(resolve, reject) {
        const id = $.randomId();
        ...
        setTimeout(() => {
            if(whispers.data[id] !== undefined) {
                console.error(`WS ${event} timed out after ${timeout}ms`);
                reject(0);
            }
        }, timeout);
    });
...
$.registerWhisperListener = function() {
