$.game('plinko', function(container, overviewData) {
    ...
    setTimeout(function() {
        const e = $.customHistoryPopover($(`<div class="plinkoCustomHistoryElement" style="background: ${colors[currentPlinkoInstance.pins][response.game.data.bucket][0]}; border-bottom: 1px solid ${colors[currentPlinkoInstance.pins][response.game.data.bucket][1]}">${response.game.multiplier}x</div>`), {
...
$.on('/game/plinko', function() {
    ...
    $.sidebar(function(component) {
        ...
        component.buttons($.lang('general.pins'))
            .add('8', function(e) {
                if(currentPlinkoInstance != null) {
                    currentPlinkoInstance.setPins(8);
                    currentPlinkoInstance.reset();
                }
            })
            .add('10', function(e) {
                if(currentPlinkoInstance != null) {
                    currentPlinkoInstance.setPins(10);
                    currentPlinkoInstance.reset();
                }
            })
            .add('12', function(e) {
                if(currentPlinkoInstance != null) {
                    currentPlinkoInstance.setPins(12);
                    currentPlinkoInstance.reset();
                }
            })
            .add('16', function(e) {
                if(currentPlinkoInstance != null) {
                    currentPlinkoInstance.setPins(16);
                    currentPlinkoInstance.reset();
                }
            })
            .add($.lang('general.edit'), function(e) {
                if(e.find('input').length === 0) {
                    e.html(`<input type="number" placeholder="${$.lang('general.pins')}">`);
                    e.find('input').on('keyup change input', function() {
                        let v = parseInt($(this).val());

                        if(isNaN(v) || v < 8 || v > 16) e.find('input').toggleClass('error', true);
                        else {
                            e.find('input').toggleClass('error', false);
                            if(currentPlinkoInstance != null) {
                                currentPlinkoInstance.setPins(v);
                                currentPlinkoInstance.reset();
                            }
                        }
                    });
                } else if(currentPlinkoInstance != null) {
                    const v = parseInt(e.find('input').val());
                    if(isNaN(v) || v < 8 || v > 16) return;
                    currentPlinkoInstance.setPins(v);
                    currentPlinkoInstance.reset();
                }

                if(isNaN(parseInt(e.find('input').val())) || parseInt(e.find('input').val()) < 8 || parseInt(e.find('input').val()) > 16) e.find('input').toggleClass('error', true);
                else {
                    e.find('input').toggleClass('error', false);
                    if(currentPlinkoInstance != null) {
                        currentPlinkoInstance.setPins(parseInt(e.find('input').val()));
                        currentPlinkoInstance.reset();
                    }
                }
            });
    ...
    reset() {
        ...
        for (let i = 0; i <= this.pins; i++) {
            for (let j = 0; j <= i; j++) {
                ...
                if(isBucket) e.html(`x${$.gameData()[this.difficulty][this.pins][j]}`).tooltip({
                    placement: 'bottom',
                    title: `x${$.gameData()[this.difficulty][this.pins][j]}`
                });
