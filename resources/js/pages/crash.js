...
const debug = false, width = 800, height = 600,
    in_progress_color = '#ffcc00', crash_color = '#ff1f44', cashoutTexts = [];
let crashed = false, placedBetThisRound = false, betValue,
    startTimestamp = 0, currentMultiplier = 1, autoCashout = 2, id;
...
$.game('crash', function(container, overviewData) {
    if(!$.isOverview(overviewData)) {
        ...
        const updateMultiplier = function() {
            if($.currentBettingType() === 'manual') $('.play-button').html($.lang('general.take', { value: (betValue * parseFloat(currentMultiplier)).toFixed(8), icon: window.Laravel.currency[$.currency()].icon}));
        };
        ...
        const startGame = function() {
            ...
            function nextMultiplier() {
                ...
                $(`[data-update="true"]`).html(`x${parseFloat(currentMultiplier).toFixed(2)}`);
            ...
            let interval = workerTimers.setInterval(function() {
        ...
        setTimeout(() => $('.play-button').addClass('disabled'), 100);
        ...
        _.forEach($.gameData().history, function(m) {
            ...
            $('.crashCustomHistory').append($.customHistoryPopover($(`<div class="crashCustomHistoryElement" style="background: ${color[0]}; border-bottom: 1px solid ${color[1]}">${m.multiplier.toFixed(2)+'x'}</div>`), {
        ...
        $.multiplayer((event, data) => {
            switch (event) {
                ...
                case 'MultiplayerGameFinished':
                    ...
                    const el = $.customHistoryPopover($(`<div class="crashCustomHistoryElement" style="background: ${color[0]}; border-bottom: 1px solid ${color[1]}">${parseFloat(currentMultiplier).toFixed(2)+'x'}</div>`), {
                    ...
                    setTimeout(() => reset($('.game-content-crash canvas')[0].getContext('2d')), 5000);
                    break;
...
function drawGrid(ctx) {
    ...
    let size = Math.max(width, height) * scale + gridScale * 2;
...
function redrawCanvas(ctx) {
    ...
    ctx.fillText(`${parseFloat(currentMultiplier).toFixed(2)}x`, width / 2, height / 2);

    const rulerY = new Ruler(ctx, 50, 50, 50, height - 50);
    const rulerX = new Ruler(ctx, 48, height - 50, width - 50, height - 50);

    const secondsRunning = startTimestamp === 0 ? 0 : parseInt((+new Date() / 1000) - startTimestamp);
    ...
    rulerY.addText(`x${(multiplierOffset(2.5, 1)).toFixed(1)}`, 50 / 2, 50);
    rulerY.addText(`x${(multiplierOffset(2.2, 2)).toFixed(1)}`, 50 / 2, 50 + (((height - 50 + (offset / 2)) / 6)));
    rulerY.addText(`x${(multiplierOffset(1.9, 3)).toFixed(1)}`, 50 / 2, 50 + (((height - 50 + (offset / 2)) / 6) * 2));
    rulerY.addText(`x${(multiplierOffset(1.6, 4)).toFixed(1)}`, 50 / 2, 50 + (((height - 50 + (offset / 2)) / 6) * 3));
    rulerY.addText(`x${(multiplierOffset(1.3, 5)).toFixed(1)}`, 50 / 2, 50 + (((height - 50 + (offset / 2)) / 6) * 4));
    rulerY.addText(`x${(multiplierOffset(1.0, 6)).toFixed(1)}`, 50 / 2, 50 + offset + (((height - 50 - (offset / 2)) / 6) * 5));
    ...
    _.forEach(cashoutTexts, function(e) {
        ...
        ctx.fillStyle = `rgba(${$.currentTheme() === 'dark' ? '255, 255, 255' : '0, 0, 0'}, ${e.alpha})`;
        ctx.font = `15px Open Sans`;
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText(`${e.name} x${e.multiplier}`, e.x, e.y + (25 - (e.alpha * 10)));
...
$(document).on('page:themeChange', function() {
    if(!window.location.pathname.includes('crash')) return;
    const ctx = $('.game-content-crash canvas')[0].getContext('2d');
    reset(ctx);
});
...
$.on('/game/crash', function() {
    ...
    $.sidebar(function(component) {
        ...
        _.forEach($.gameData().players, (data) => $.multiplayerBets().add(data.user, data.game));
    });
}, ['/css/pages/crash.css']);
