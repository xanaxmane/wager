...
class User extends \Jenssegers\Mongodb\Auth\User implements JWTSubject {
     ...
     */
    protected $hidden = [
        'password', 'remember_token', 'email', 'ignore', 'ban', 'mute',
        'discord_bonus', 'notification_bonus', 'latest_activity', 'jwt',
        'tfa', 'tfa_enabled', 'tfa_persistent_key', 'tfa_onetime_key', 'email_notified', 'dismissed_global_notifications',
    ...
    public function chatChannel(): string {
        ...
        if($channel == null) {
            $channel = 'casino_'.app()->getLocale();
            $this->update(['chat_channel' => $channel]);
        }
    ...
    public function tfa(): TwoFactorAuth {
        return new TwoFactorAuth('win5x.com/'.$this->name);
    }
